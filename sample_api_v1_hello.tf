...
// POST
resource "aws_api_gateway_method" "SampleAPI-POST-v1-hello" {
  rest_api_id   = "${aws_api_gateway_rest_api.SampleAPI.id}"
  resource_id   = "${aws_api_gateway_resource.SampleAPI-v1-hello.id}"
  http_method   = "POST"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "SampleAPI-POST-v1-hello" {
  rest_api_id = "${aws_api_gateway_rest_api.SampleAPI.id}"
  resource_id = "${aws_api_gateway_resource.SampleAPI-v1-hello.id}"
  http_method = "${aws_api_gateway_method.SampleAPI-POST-v1-hello.http_method}"
  type        = "MOCK"
...
resource "aws_api_gateway_method_response" "SampleAPI-POST-v1-hello-200" {
  rest_api_id = "${aws_api_gateway_rest_api.SampleAPI.id}"
  resource_id = "${aws_api_gateway_resource.SampleAPI-v1-hello.id}"
  http_method = "${aws_api_gateway_method.SampleAPI-POST-v1-hello.http_method}"
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "SampleAPI-POST-v1-hello-200" {
  rest_api_id = "${aws_api_gateway_rest_api.SampleAPI.id}"
  resource_id = "${aws_api_gateway_resource.SampleAPI-v1-hello.id}"
  http_method = "${aws_api_gateway_method.SampleAPI-POST-v1-hello.http_method}"
  status_code = "${aws_api_gateway_method_response.SampleAPI-POST-v1-hello-200.status_code}"

  response_templates {
    "application/json" = "${file("json/SampleAPI/v1/hello/post-200.json")}"
  }
}
